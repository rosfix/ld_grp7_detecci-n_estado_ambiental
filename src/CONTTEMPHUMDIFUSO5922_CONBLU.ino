...
String menSalida,Temp,Hum;
char op;

// Setup 
void setup()
{
    //Coloca aqui las entradas desde el sensor de temperatura y humedad
    //pinMode(sensor,INPUT);
    Serial.begin(9600);
    Serial1.begin(9600);//PARA EL BLUETOOH
    //dht.begin();
...
{
    Serial1.println(" ");
    Serial1.println("CURSO:Logica Difusa y Sistemas de Control-GRUPO7");
    Serial1.println("Ingrese (1) para ingreso MANUAL");
    Serial1.println("Ingrese (2) para Lectura del SENSOR");
    delay(10000);
    while (Serial1.available()){
      ...
      if (op=='2'){
        //LECTURA DEL SENSOR DHT11
            ...
            setearSalida(temperatura, humedad, g_fisOutput[0]);
      }else{
            if(op=='1'){
              Serial.println("HA ELEGIDO INGRESAR MANUALMENTE");
              Serial1.println("HA ELEGIDO INGRESO MANUAL");
              Serial1.print("INGRESE HUMEDAD");
              Serial.print("HUMEDAD = ");
              delay(10000);
              humedad=Serial1.parseFloat();
              Serial1.println(humedad);
              Serial.println(humedad);
              
              Serial1.print("INGRESE TEMPERATURA");
              Serial.print("TEMPERATURA = ");
              delay(10000);
              temperatura=Serial1.parseFloat();
              Serial1.println(temperatura);
              Serial.println(temperatura);
              if(humedad>=0 && humedad<=100 && temperatura>0 && temperatura<=42 & temperatura!=0 & humedad!=0){
                    g_fisInput[0] = temperatura;
                    g_fisInput[1] = humedad;
                    g_fisOutput[0] = 0;
                    fis_evaluate();
                ...
                Serial1.println("INGRESO FUERA DE RANGO");
              }
            }else{
              Serial1.println("INGRESO FUERA DE RANGO");
            }
 ...
 void setearSalida(float temperatura,float humedad,float salida){
           String menSalida;
            Serial.print("Salida: ");
            ...
            if(salida>=0 && salida<=0.4){
               if(salida>=0.3 && salida<=0.4){
                ...
                Serial1.println("-CLIMA ENTRE MALO Y NORMAL");
                menSalida=" CLIMA:MyN";
               ...
               if(salida>=0.41 && salida<=0.5){
                ...
                analogWrite(azul,0);
                  ...
                  if(salida>=0.5 && salida<=0.7){
                    ...
                    Serial1.println("-CLIMA ENTRE NORMAL Y BUENO ");
                    menSalida=" CLIMA:NyB";
                    analogWrite(rojo,0);
                    analogWrite(verde,255);
...
//Punteros a las implementaciones de funciones miembro
_FIS_MF fis_gMF[] =
{
    fis_trimf
};
...
FIS_TYPE fis_gIMax[] = { 50, 100 };//CAMBIAR EL RANGO 30 POR 42 GRADOS
